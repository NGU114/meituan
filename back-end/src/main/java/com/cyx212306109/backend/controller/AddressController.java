...
public class AddressController {
    ...
    public ApiResponse<AddressDto.AddressResponse> update(@PathVariable Long addressId,
                                                          @Valid @RequestBody AddressDto.UpdateRequest request) {
        return ApiResponse.ok("地址更新成功", addressService.update(addressId, request));
    }

    @PostMapping("/{addressId}/default")
    public ApiResponse<AddressDto.AddressResponse> makeDefault(@PathVariable Long addressId) {
        return ApiResponse.ok("默认地址已更新", addressService.makeDefault(addressId));
    }
